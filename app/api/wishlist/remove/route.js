...
export async function DELETE(req) {
    ...
    try {
        ...
        await wishlistDAO.removeFromWishlist(gameID, userID);

        return NextResponse.json(
            { message: "Videoigra uklonjena s liste želja!" },
            { status: 200 }
        );
    } catch (error) {
        console.error("Interna pogreška prilikom uklanjanja videoigre:", error);
        return NextResponse.json(
            { error: "Interna pogreška prilikom uklanjanja videoigre!" },
            { status: 500 }
        );
    }
}
